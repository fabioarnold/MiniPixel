...
static void glnvg__getUniforms(GLNVGshader* shader)
{
	...
	shader->loc[GLNVG_LOC_COLORMAP] = glGetUniformLocation(shader->prog, "colormap");
							   ...
							   NVGscissor* scissor, float width, float fringe, float strokeThr)
{
	...
	NVGcolor premulInnerCol = glnvg__premulColor(paint->innerColor);
	NVGcolor premulOuterCol = glnvg__premulColor(paint->outerColor);
	frag->innerCol[0] = premulInnerCol.rgba[0];
	frag->innerCol[1] = premulInnerCol.rgba[1];
	frag->innerCol[2] = premulInnerCol.rgba[2];
	frag->innerCol[3] = premulInnerCol.rgba[3];
	frag->outerCol[0] = premulOuterCol.rgba[0];
	frag->outerCol[1] = premulOuterCol.rgba[1];
	frag->outerCol[2] = premulOuterCol.rgba[2];
	frag->outerCol[3] = premulOuterCol.rgba[3];
...
static void glnvg__setUniforms(GLNVGcontext* gl, int uniformOffset, int image, int colormap)
{
	GLNVGtexture* tex = NULL;
#if NANOVG_GL_USE_UNIFORMBUFFER
	...
	if (colormap != 0) {
		tex = glnvg__findTexture(gl, colormap);
		if (tex != NULL) {
...
static void glnvg__fill(GLNVGcontext* gl, GLNVGcall* call)
{
	...
	glnvg__setUniforms(gl, call->uniformOffset, 0, 0);
	...
	glnvg__setUniforms(gl, call->uniformOffset + gl->fragSize, call->image, call->colormap);
...
static void glnvg__convexFill(GLNVGcontext* gl, GLNVGcall* call)
{
	...
	glnvg__setUniforms(gl, call->uniformOffset, call->image, call->colormap);
...
static void glnvg__renderFlush(void* uptr)
{
	...
	if (gl->ncalls > 0) {
		...
		glUniform1i(gl->shader.loc[GLNVG_LOC_COLORMAP], 1);
