...
pub fn init(allocator: Allocator, editor_widget: *EditorWidget) !*Self {
    ...
    self.* = Self{
        ...
        .width_label = try gui.Label.init(allocator, Rect(f32).make(10, 20, 0, 20), "Width:"),
        .width_spinner = try gui.Spinner(i32).init(allocator, Rect(f32).make(55, 20, 60, 20)),
        .height_label = try gui.Label.init(allocator, Rect(f32).make(125, 20, 0, 20), "Height:"),
    ...
    self.width_spinner.setValue(@intCast(i32, editor_widget.document.width));
    self.height_spinner.setValue(@intCast(i32, editor_widget.document.height));
